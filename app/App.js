...
export default class App extends Component<Props> {
	...
	doLogin = () => {
		...
		sendRequestForJson(
			'https://typo3.jmwd.de',
			`/?user=${this.state.loginData.username}&pass=${this.state.loginData.password}&logintype=login&pid=3&type=5000`,
	...
	loginCallBack = ( response ) => {
		...
		if ( typeof response.userid != "undefined" && response.userid > 0 ) {
			...
			Keychain.setGenericPassword( this.state.loginData.username, this.state.loginData.password );
		} else {
			...
			case 'dashboard':
				return (
					<View style={styles.container}>
						...
						<Image source={{ uri: this.state.userData.image }}
						       style={{
							       width: 200,
							       height: 200,
							       borderRadius: 100,
